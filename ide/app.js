...
import { DoomSim, doomKey } from './doom-sim.js';
...
async function openFile(path) {
  if (!state.open.has(path)) {
    ...
    try {
      ...
      line('build', `could not open ${path}: ${e.message}`, 'err');
      return;
    }
...
async function firmwareImage() {
  ...
  if (b && b.ok && b.binary) {
    return { bytes: Uint8Array.from(atob(b.binary), (c) => c.charCodeAt(0)), from: 'your latest build' };
  }
  const r = await fetch(`firmware/prebuilt/${combo()}.bin`, { cache: 'no-store' });
  if (!r.ok) return null;
  return { bytes: new Uint8Array(await r.arrayBuffer()), from: `prebuilt firmware/prebuilt/${combo()}.bin` };
}
